...
export default function LandingPage() {
  ...
  return (
    <main className="min-h-screen bg-[#0a0f1a] text-white selection:bg-cyan-500/30 font-sans overflow-x-hidden">
      
      {/* 1. NAVBAR FIXE */}
      <nav className="fixed top-0 w-full z-50 border-b border-white/5 bg-[#0a0f1a]/50 backdrop-blur-md px-6 py-5 flex justify-between items-center">
        ...
        <div className="hidden md:flex gap-10 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">
          ...
          <a href="#forum" className="hover:text-cyan-400 transition-colors">Communauté</a>
        ...
        <div className="flex items-center gap-4">
          ...
          <Link href="/signup" className="text-[10px] font-black bg-cyan-400 text-[#0a0f1a] px-6 py-2.5 rounded-full hover:bg-white transition-all uppercase tracking-tighter shadow-xl shadow-cyan-400/20">
            S'inscrire
          </Link>
        </div>
      </nav>
      ...
      <section className="relative min-h-screen flex flex-col items-center justify-center pt-20">
        {/* Glow de fond adapté à la couleur sombre */}
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_50%,rgba(34,211,238,0.05),transparent)] z-0" />
        
        <motion.div {...fadeUp} className="z-10 text-center px-4 max-w-5xl">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-cyan-400/10 border border-cyan-400/20 rounded-full text-cyan-400 text-[10px] font-black uppercase tracking-[0.3em] mb-10">
          ...
          <div className="flex flex-col md:flex-row gap-6 justify-center items-center">
            <Link href="/signup">
              <motion.button 
                ...
                whileTap={{ scale: 0.95 }}
                className="bg-white text-[#0a0f1a] px-12 py-6 rounded-full font-black text-sm uppercase tracking-widest shadow-2xl shadow-white/5 flex items-center gap-3"
              >
            ...
            <div className="flex -space-x-3">
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="w-10 h-10 rounded-full border-2 border-[#0a0f1a] bg-slate-800" />
              ))}
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-8">
          {[
            ...
            <motion.div 
              ...
              className="p-10 bg-slate-900/30 border border-white/5 rounded-[2.5rem] hover:bg-cyan-400/10 transition-all group"
            >
      ...
      <section id="forum" className="py-32 px-6 bg-[#0a0f1a]">
        ...
        <div className="max-w-4xl mx-auto space-y-4">
          {[
            ...
            <motion.div 
              key={i}
              whileHover={{ x: 15, backgroundColor: "rgba(34, 211, 238, 0.05)" }}
            ...
            >
              <div className="flex gap-6 items-center">
                <div className="w-12 h-12 rounded-2xl bg-cyan-400/10 flex items-center justify-center font-bold text-cyan-400 text-xl italic">#</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              >
                ...
                <div className="w-20 h-20 bg-gradient-to-br from-cyan-400 to-cyan-600 rounded-[2rem] mb-8 shadow-2xl shadow-cyan-400/20" />
                <h4 className="text-2xl font-black mb-1 italic uppercase">{mentor.name}</h4>
                <p className="text-cyan-400 text-[10px] font-black uppercase tracking-[0.2em] mb-6">{mentor.role}</p>
      ...
      <section className="py-40 px-6 text-center">
        <motion.div 
          ...
          className="max-w-5xl mx-auto p-20 bg-gradient-to-br from-[#111827] to-[#0a0f1a] border border-white/5 rounded-[4rem] relative overflow-hidden shadow-2xl"
        >
          <div className="relative z-10">
            <h2 className="text-5xl md:text-8xl font-black mb-10 italic uppercase tracking-tighter leading-none text-white">Prêt pour <br/><span className="text-cyan-400">l'excellence ?</span></h2>
            <Link href="/signup">
              <button className="bg-cyan-400 text-[#0a0f1a] px-16 py-7 rounded-full font-black uppercase tracking-widest hover:bg-white transition-all text-sm shadow-2xl shadow-cyan-400/20">
                Commencer l'aventure
              </button>
            </Link>
          </div>
          {/* Décoration de fond subtile */}
          <div className="absolute top-0 right-0 w-96 h-96 bg-cyan-400/5 rounded-full blur-3xl -mr-32 -mt-32" />
        </motion.div>
      </section>
