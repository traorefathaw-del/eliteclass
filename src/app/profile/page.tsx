...
import { Mail, Shield, Calendar, Award, Zap, Loader2, UserCircle2 } from "lucide-react";
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto space-y-8">
      <header className="flex flex-col md:flex-row items-center gap-6 mb-12">
        ...
        <div className="text-center md:text-left">
          ...
          <p className="text-slate-500 font-mono text-xs uppercase tracking-[0.3em] mt-2">
            // STATUS: NOUVEAU MEMBRE • ID: {user?.id?.slice(0, 8)}
          </p>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          { icon: <Zap size={24} className="text-slate-600" />, val: "0", label: "Points d'XP" },
          { icon: <Award size={24} className="text-slate-600" />, val: "0", label: "Badges Acquis" },
          { icon: <Calendar size={24} className="text-slate-600" />, val: "0h", label: "Temps de Formation" }
        ].map((stat, i) => (
